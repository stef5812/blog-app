...
function LinksDropdown({ theme, mobile = false }) {
  const [open, setOpen] = useState(false);
    ...
    <div className={mobile ? "relative w-full" : "relative"}>
      ...
      {open && (
        <div
          className={
            mobile
              ? "mt-2 w-full overflow-hidden rounded-2xl border border-zinc-200/80 bg-white/95 shadow-[0_12px_40px_rgba(0,0,0,0.12)] ring-1 ring-zinc-200/70 backdrop-blur-md"
              : "absolute right-0 top-full z-50 mt-2 w-64 overflow-hidden rounded-2xl border border-zinc-200/80 bg-white/95 shadow-[0_12px_40px_rgba(0,0,0,0.12)] ring-1 ring-zinc-200/70 backdrop-blur-md"
          }
        >
...
export default function SiteHeader({ me, setMe }) {
  ...
  <div className="space-y-2 pb-4 md:hidden">
    ...
    <div className="block md:hidden">
      <LinksDropdown theme={theme} mobile />
    </div>
    ...
    {isLoggedIn ? (
      ...
      <a
        href={loginHref}
        className="block w-full rounded-xl bg-white/90 px-4 py-2 text-sm font-semibold text-slate-900 shadow-sm transition hover:bg-white"
      >
        Login
      </a>
    )}
